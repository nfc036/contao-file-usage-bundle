...
class FileUsageHelper extends Backend
{
  private static $FILTER = ['tl_files', 'tl_search', 'tl_search_index', 'tl_undo', 'tl_version', 'tl_log', 'tl_user', 'tl_user_group', 'tl_trusted_device',
    ...
    public function updateFileUsages()
    {
      ...
      // System::log('FileUsageHelper:updateFileUsages(), count='.$count, __METHOD__, TL_CRON);
      $updated = $this->updateFiles($count);
      // Add a log entry
      System::log(sprintf("File references updated (%d of %d files) [ContaoFileUsageBundle]", $updated, $count), __METHOD__, TL_CRON);
    }
  ...
  public function updateFiles($count = 10) {
    $updated = 0;
    $arrFiles = $this->Database->prepare(sprintf("SELECT * FROM tl_files WHERE type='file' and (nfu_lastcheck is null or nfu_lastcheck < UNIX_TIMESTAMP() - 3600) order by nfu_lastcheck asc, LOWER(path) asc limit 0, %d", $count))
      ->execute()
      ->fetchAllAssoc();
    if (count($arrFiles) > 0) {
